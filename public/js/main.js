...
// --- Section reveal ---------------------------------------------------------
// Sections settle in as they scroll into view. The .reveal class is added here
// rather than in the HTML on purpose: if this script never runs, or the browser
// has no IntersectionObserver, nothing is ever hidden in the first place. A
// reveal that fails leaves a blank page, which is a bad trade for some polish.
(function () {
  if (!('IntersectionObserver' in window)) return;
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

  // The hero is above the fold — fading it in would just look like a slow page.
